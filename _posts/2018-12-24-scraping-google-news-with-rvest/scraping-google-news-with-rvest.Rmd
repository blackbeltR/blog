...
This work is the result of a question that appeared on the WhatsApp group of our site [**blackbeltR**](https://www.blackbeltR.com.br){target="_blank"}. A user came up with this problem and I decided to help him. It was a cool challenge, so why not?
  
  A great deal of the basic ideas comes from his own code. I just kept it and added few things in order to get the code working. First off, you should take a look at the Google News (or Google Notícias) website [HERE](https://news.google.com/?hl=pt-BR&gl=BR&ceid=BR:pt-419){target="_blank"} which I reproduce below:
...
---
  ...
  You may notice, on the right side of the page, that we are using the Google Chrome **dev-tools**. We use this to identify the *html nodes* we need. You can access this tool by hitting the **F12** key. The html nodes are passed as arguments to the `rvest` functions.
...
In this last case we used a regular expression (REGEX) to clean up the data. We did this by separating the actual headline phrases from the complementary ones. In some cases, we have a phrase ending with uppercase letters such as "SP" (a Brazilian state) collapsed with other phrase initiating with another uppercase letter such as "A" (a common article in Portuguese) for example. We have to think of a better way to split these cases, but the current result is quite satisfactory for now. 

Before we finish, we have to clean up our data. It is common to collect garbage in the process such as data related to "fake news checking", which is a section on the right side of the page (in the Brazilian version). As a result, it is possible that the three vectors we have created may have different sizes. Therefore, we use the smallest of them as the base and just delete the entries above this number on the other two vectors.
