...
Now is when **emstreeR** comes in handy. We pass the point columns from the df_location object and it computes the minimum route between the points, e.g. the Euclidean minimum spanning tree.
...
```{r, preview=TRUE, fig.height=4, fig.width=8, echo=TRUE, message=FALSE}
## Plot:
...
Now that you have the information about the minimum route, depending on the number of planes your company has, you can choose the regions and cities each plane will fly to. Clustering is another common application of the Minimum Spanning Tree problem. Nonetheless, let's save that topic for another post.

Below, I will show you another examples using **emstreeR**.
...
The `stat_MST()` extension also makes it easy plotting 2D plots in ggplot2's style:
...
You can also make 3D plots with **emstreeR**. It has a wrapper to the 'scatterplot3d' function.
...
But, again, if you prefer fancier plots, we can also do a 3D MST plot using plotly:
